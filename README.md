...
本仓库基于 [yawiii/ComfyUI-Prompt-Assistant](https://github.com/yawiii/ComfyUI-Prompt-Assistant) 修改而来，遵循原项目 **GPL-3.0** 开源协议发布。所有功劳归于原作者 [@yawiii](https://github.com/yawiii)，本仓库仅在原版基础上做了少量功能扩展，其余功能、文档与原版保持一致。

### 本版修改：图像反推节点支持最多 9 张图合并反推（动态输入口）

原版「✨Image Caption (VLM) 图像反推」节点只支持单张图片输入，本版为其扩展了多图能力，最高支持 9 张图（`image` + `image2` ~ `image9`）：

* **动态输入口**：节点创建时只显示 `image` 一个输入口；接入 `image` 后自动出现 `image2`，接入 `image2` 后自动出现 `image3`……以此类推，最高到 `image9`。断开某个输入时，其后的输入口自动级联回收。
* **合并模式**：接入多个图像时，所有已连接的图像会合并进**一次** VLM 请求，由模型综合理解后输出一条整体描述（适合多参考图场景，如分镜组图、风格参考合集）。
* **原行为不变**：只接 `image` 时，行为与原版完全一致（单张反推，或 IMAGE batch 逐帧独立反推）。
* **输出兼容**：输出仍为 `caption_text`（完整文本）与 `caption_list`（字符串列表），下游节点无需任何修改。
* **智能截断**：图片数量超过当前模型的多图上限时，自动按模型上限截断并在控制台打印提示，不会中断任务。
* **缓存正确**：额外图像参与节点缓存指纹计算，更换任意一张图都会正确触发重新反推。
* **工作流兼容**：保存的工作流会按名称恢复已连接的多图输入口，加载后状态与保存时一致。
* **改动范围**：`node/image_caption_node.py`（后端多图合并逻辑）与 `js/node/dynamicImageInputs.js`（前端动态输入口，新增文件），服务层复用原版现成的多图接口（`VisionService.analyze_images`）。
